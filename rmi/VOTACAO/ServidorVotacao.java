...
// UnicastRemoteObject possui um construtor que, ao ser chamado (via super()), faz o registro do objeto como um objeto remoto ativo.
// Esse processo cria um socket de servidor que fica ouvindo conexões RMI.
// O RMI gera um stub (proxy) para esse objeto, que implementa a mesma interface remota.
// Esse stub sabe:
// O endereço IP e a porta do servidor;
// Como serializar parâmetros e resultados;
// Como abrir conexão e invocar o método real no servidor.
// O stub é o que será enviado ao RMI Registry quando você fizer o rebind().
// Internamente, cada objeto remoto exportado via UnicastRemoteObject recebe um RemoteRef (referência remota).
// Isso permite que o RMI saiba qual instância do objeto remoto o cliente está chamando, mesmo que haja vários objetos no mesmo servidor.
//extends UnicastRemoteObject é uma forma prática de dizer ao Java RMI que o seu objeto deve ser exportado automaticamente como remoto, pronto para aceitar chamadas vindas de clientes e gerar o stub correspondente.

public class ServidorVotacao extends UnicastRemoteObject implements ServicoVotacao { // O objeto passa a “ouvir” requisições de métodos vindas da rede
    private static final long serialVersionUID = 1L;
    
    // Armazenamento seguro
    // [4] private Map<...>: Estes são os "bancos de dados" do servidor. Em um app real, isso estaria em um SQL, mas para RMI, o princípio é o mesmo. Note que são todos private. O mundo exterior só pode acessá-los através dos métodos da interface RMI.
    ...
    public ServidorVotacao() throws RemoteException {
        super(); // exporta automaticamente o objeto remoto É o que liga a "escuta" de rede para este objeto
        ...
        // new ConcurrentHashMap<>(); Inicializa os mapas thread-safe.
        this.cpfsQueVotaram = Collections.synchronizedSet(new HashSet<>()); //zona critica
    ...
    private void cadastrarEleitor(String cpf, String senha, boolean admin) {
        String senhaHash = gerarHash(senha); //Medida de segurança. O servidor armazena apenas o hash da senha, nunca a senha original.
    ...
    public synchronized String autenticar(String cpf, String senha, boolean admin) throws RemoteException {
        ...
        String token = UUID.randomUUID().toString(); // Gera um token único universal. Este token é a "chave" do cliente para provar que está logado.
    ...
    // garante atomicidade. O bloco registrarVoto (verificar se já votou, adicionar voto, marcar como votado) não pode ser interrompido no meio.
    @Override
    ...
    private String gerarHash(String texto) { // implementação da criptografia SHA-256.
        try {
