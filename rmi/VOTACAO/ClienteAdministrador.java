...
// Este arquivo é quase idêntico ao ClienteEleitor na sua estrutura, o que prova a eficiência do design:
// Ele também tem uma variável private ServicoVotacao servico;.
// Ele conectar() e faz o lookup("ServicoVotacao") exatamente da mesma forma.
// A única diferença é o que ele faz:
// No autenticar(), ele chama servico.autenticar(login, senha, true); (com true para admin).
// No menu, ele chama os métodos de admin: servico.iniciarEleicao(token), servico.obterResultadoParcial(token), etc.
// Ele usa o mesmo Stub ServicoVotacao para chamar um conjunto diferente de métodos. A lógica de autorização (verificar se o token pertence a um admin) é toda feita no servidor, que é onde deve estar.
